...
void filtering()
{
  ...
  //Scalar lowerb = Scalar(2, 10, 0);//Scalar(30,127,51);
  //Scalar upperb = Scalar(12, 255, 90);//Scalar(82,255,255);

  //inRange(src,lowerb,upperb,mask);

  /*
  Compute the average of all pixels and compare if some pixel stands out.
  This pixel could be our letter.
  */
  mask = Mat::zeros( src.rows, src.cols, CV_8UC1 );
  debug = Mat::zeros( src.rows, src.cols, CV_8UC1 );
  double averager = 0;
  double averageg = 0;
  double averageb = 0;
  ...
  for(int i =  0; i < src.rows; i++)
  {
    ...
    {
      averager += src.at<Vec3b>(i,j)[0];
      averageg += src.at<Vec3b>(i,j)[1];
      averageb += src.at<Vec3b>(i,j)[2];
    }
  }
  averageb /= src.cols*src.rows;
  averageg /= src.cols*src.rows;
  averager /= src.cols*src.rows;
  //cout << averager << " " << averageg << " " << averageb << '\n';
  ...
  for(int i = 0; i < src.rows; i++)
  {
    for(int j = 0; j < src.cols; j++)
    {
      ...
      double distance = sqrt(pow(r-averager,2) + pow(b-averageb,2) + pow(g-averageg,2));

      debug.at<uchar>(i,j) = distance;
      if(distance > 80)
      {
  ...
  Mat kernel = Mat::ones(8*scaling,8*scaling,CV_8UC1);
...
//imshow("Mask",mask);
  ...
  //Debugging
  Mat fll = Mat::zeros( src.rows, src.cols, CV_8UC3 );
    ...
    if(event == EVENT_RBUTTONDOWN)
     {
       ...
       */
       ...
       {
         maxa = Point(x,y);
         circle(img,maxa,5,Scalar(255,255,255));
       }
       if(here == 2)
       {
         maxb = Point(x,y);
         circle(img,maxb,5,Scalar(255,255,255));
       }
       if(here == 3) {
         rmva = Point(x,y);
         circle(img,rmva,3,Scalar(0,0,255));
       }
       if(here == 4)
       {
         rmvb = Point(x,y);
         circle(img,rmvb,3,Scalar(0,0,255));
         doneselect = true;
       }
